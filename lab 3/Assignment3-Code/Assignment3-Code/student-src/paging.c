...
void proc_init(pcb_t *proc) {
    ...
    pfn_t vframe;
    vframe = free_frame();
    memset(mem + vframe * PAGE_SIZE, 0, PAGE_SIZE);
     ...
     */
    proc->saved_ptbr = vframe;
    (frame_table + proc->saved_ptbr)->protected = 1;
    (frame_table + proc->saved_ptbr)->mapped = 1;
}
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    vpn_t v_vpn = vaddr_vpn(address);
    uint16_t v_offset = vaddr_offset(address);


    /* If an entry is invalid, just page fault to allocate a page for the page table. */
    pte_t* p_entry  = (pte_t*) (mem + (PTBR * PAGE_SIZE)) + v_vpn;
    if (p_entry->valid == 0) {
        page_fault(address);
    }

    /* Set the "referenced" bit to reduce the page's likelihood of eviction */
    //frame_table->process = current_process;
    (frame_table + p_entry->pfn)->referenced = 1;
    //frame_table->vpn = v_vpn;
    ...
    */
    paddr_t phys_addr;
    phys_addr = (paddr_t) ((p_entry->pfn) * PAGE_SIZE) + v_offset;
    //phys_addr = ((paddr_t) (mem + (p_entry->pfn))) + v_offset; //if doesnt work, remove *
    /* Either read or write the data to the physical address
       depending on 'rw' */
    if (rw == 'r') {
        ++stats.reads;
        return mem[phys_addr]; //return the address or data present in address
    } else {
        ++stats.writes;
        p_entry->dirty = 1;
        mem[phys_addr] = data;
...
void proc_cleanup(pcb_t *proc) {
    ...
    pte_t* page_entry = (pte_t*) (mem + (proc->saved_ptbr * PAGE_SIZE));

    /* Iterate the page table and clean up each valid page */
    for (size_t i = 0; i < NUM_PAGES; i++) {
        if ((page_entry + i)-> valid == 1) {
            (frame_table + (page_entry + i)->pfn)->mapped = 0;
            (frame_table + (page_entry + i)->pfn)->protected = 0;
        }
        if (swap_exists(page_entry + i) == 1) {
            swap_free(page_entry + i);
        }
    }
