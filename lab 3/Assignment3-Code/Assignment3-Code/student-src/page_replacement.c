...
pfn_t free_frame(void) {
     ...
     */
    if ((frame_table + victim_pfn)->mapped ==  1 && (frame_table + victim_pfn)->protected == 0) {
        pcb_t* proc;
        proc = (frame_table + victim_pfn)->process;
        pte_t *pagetable;
        pagetable = (pte_t*) (mem + (proc->saved_ptbr * PAGE_SIZE)) + (frame_table + victim_pfn)->vpn;
        if (pagetable->dirty) {
            stats.writebacks++;
            pagetable->dirty = 0;
